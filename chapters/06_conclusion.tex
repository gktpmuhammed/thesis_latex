...
The main empirical conclusion is concise: in this setting, token budget under a stable visual-language interface is more decisive than adding architectural complexity. Within MedGemma ablations under matched decoding/training conditions, the strongest variant is the stable-prefix 8-token model (Base-8T), which outperforms the matched 1-token baseline on the primary global metrics used in this thesis. The new head-averaged attention case studies (lung and kidney) are consistent with this ranking and show stronger in-mask concentration for Base-8T than heavier variants in the same representative patient. Cross-decoder analysis also shows that stronger aggregate lexical-semantic metrics can coexist with higher template reuse, so deployment-oriented interpretation must include diversity and structure diagnostics in addition to metric averages.
