...
The main empirical conclusion is concise: in this setting, token budget under a stable visual-language interface is more decisive than adding architectural complexity. Within MedGemma ablations under matched decoding/training conditions, the strongest variant is the stable-prefix 8-token model (Base-8T) when ranking is anchored on GREEN and RadGraph; it outperforms the matched 1-token baseline on these and most lexical-semantic metrics, but not on RadCliQ. The new head-averaged attention case studies (lung and kidney) are consistent with this ranking and show stronger in-mask concentration for Base-8T than heavier variants in the same representative patient. In the protocol-matched decoder table (Table~\ref{tab:decoder_comparison}), the fVLM-aligned BERT-base row is highest on both GREEN and RadGraph, while BioBART is strongest on RadCliQ. Cross-decoder analysis also shows that stronger aggregate lexical-semantic metrics can coexist with higher template reuse, so deployment-oriented interpretation must include diversity and structure diagnostics in addition to metric averages.
