...
The implemented framework combined organ-level supervision via report decomposition, organ-conditioned visual token extraction, and decoder adaptation under a consistent evaluation protocol. The evidence now includes systematic MedGemma ablations and completed GPT-2 and BioBART decoder rows under the same evaluation pass.
...
\begin{itemize}
    ...
    \item a protocol-matched cross-decoder comparison with completed MedGemma, GPT-2, and BioBART rows, plus a reserved fVLM-aligned BERT-base baseline row.
\end{itemize}
...
\begin{enumerate}
    \item Finalize the fVLM-aligned BERT-base run and populate the remaining decoder row under the same standardized metric pass.
...
The thesis shows that organ-aware 3D report generation is tractable when anatomical structure is treated as a first-class modeling constraint. With the current framework and results, the project is technically mature enough to finalize the remaining BERT baseline comparison and move toward stronger clinical validation.
